...
public class IncExpr extends Expr implements SimpleStmt {
    ...
    public Type doCheckType(Checker checker, Type outerType) {
        ...
        if (!(xt instanceof Basic))
            throw new IllegalStateException("Type error: illegal type for operation: increment");

        return switch ((Basic) xt) {
            case INT -> Basic.INT;
            case FLOAT -> Basic.FLOAT;
            default -> throw new IllegalStateException("Type error: illegal type for operation: increment");
        };
    }
    ...
    public String toTreeString(int level, String prefix) {
        ...
        sb.append(prefix).append(t).append(link).append(isPreOrPost() ? "Pre" : "Post").append("Increase")
